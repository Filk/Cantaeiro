class Plantitura
{
  ...
  void tocaPlantitura()
  {
    for (int i=0; i<setSequencia.length; i++)
    {
       if(i>0)
       {
       prevSample= setSequencia[i-1]-1;
       }
      
       int sampleActual= setSequencia[i]-1;
       
       if(i>0)
       {
         if (tocaSamples[prevSample].fimSample)
         {
         println("terceiro");
         ...
         //sample box 
         fill(10,200,10);
         rect(ps[sampleActual%3].x,ps[sampleActual%3].y+ps[sampleActual%3].alturaOpen+((sampleActual%3*ps[sampleActual%3].espacamentoEntreBlocos)+ps[sampleActual%3].alturaOpen), ps[sampleActual%3].comprimentoBloco, ps[sampleActual%3].alturaBloco);
